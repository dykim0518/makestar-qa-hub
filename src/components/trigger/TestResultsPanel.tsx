    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        {triggered && (
          <button
            onClick={onRefresh}
            className="rounded p-1.5 text-[var(--muted)] hover:text-white hover:bg-white/5 transition-colors"
            title="새로고침"
          >
            <svg
      ...
      {/* 폴링 중지 알림 */}
      {pollingStopped && (
        <div className="mb-3 flex items-center justify-between rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-2.5">
      ...
      {!triggered ? (
        ...
        <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-10 text-center">
          <svg
            className="mx-auto mb-3 h-6 w-6 animate-spin text-indigo-400"
            viewBox="0 0 24 24"
            fill="none"
          >
            <circle
          ...
          <div className="max-h-[calc(100vh-220px)] space-y-1 overflow-y-auto pr-1">
            {testCases.length === 0 && isRunning && (
              <div className="rounded-lg border border-dashed border-[var(--card-border)] bg-[var(--card)] p-8 text-center">
                <svg
                  className="mx-auto mb-2 h-5 w-5 animate-spin text-indigo-400"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <circle
